...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const triggerSOS = async (reason: string) => {
    ...
    // 2. Auth Check (Soft)
    if (!auth.currentUser) {
       console.warn("SOS triggered without active Firebase Session. Attempting local only.");
    }
    ...
    // 5. OPTIMISTIC UI UPDATE: Trigger Red Screen IMMEDIATELY
    // We do this BEFORE network calls so the user feels safe even if network fails.
    onAlert(log);
    setErrorMsg(null);

    // 6. Network Broadcast (Best Effort)
    try {
      ...
      // Save Alert Log to User Profile
      broadcastTasks.push(
        set(ref(rtdb, `users/${currentUser.id}/alerts/${alertId}`), log)
      );
      
      await Promise.all(broadcastTasks);

    } catch (err: any) {
      console.error("SOS Network Failure:", err);
      // We do NOT revert the alert. We just warn the user.
      const isPermError = err.message?.includes("PERMISSION") || err.code?.includes("PERMISSION");
      if (isPermError) {
        setErrorMsg("⚠️ NETWORK SEND FAILED (PERMISSIONS). ALERT IS ACTIVE LOCALLY.");
      } else {
        setErrorMsg("⚠️ NETWORK SEND FAILED. CHECK CONNECTION.");
      }
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in px-6">
        ...
        <p className="text-xs font-bold text-red-400 uppercase tracking-widest max-w-[280px] leading-relaxed mb-10">
          Aegis is transmitting your location to your guardian network.
        </p>
        <button 
