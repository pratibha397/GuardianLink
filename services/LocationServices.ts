...
import { startLocationWatch, stopLocationWatch, getPreciseCurrentPosition } from '../services/LocationServices';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const triggerSOS = async (reason: string) => {
    ...
    try {
      ...
      // Explicitly check if location was missing and warn user
      if (!loc) {
      ...
      // Ensure we don't show raw "Permission Denied" if it somehow bubbled here
      if (err.message && (err.message.includes("Permission") || err.message.includes("denied"))) {
         setErrorMsg("⚠️ ALERT SENT (GPS BLOCKED). CHECK BROWSER PERMISSIONS.");
      } else {
